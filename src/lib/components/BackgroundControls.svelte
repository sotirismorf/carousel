<script lang="ts">
	...
	let {
		...
		onThemeChange: (theme: Settings['gradientTheme']) => void;
		onColorCountChange: (count: number) => void;
		onBgImageUpload: (event: Event & { currentTarget: HTMLInputElement }) => void;
	} = $props();
</script>
	...
	<div class="flex">
		{#each [['solid', 'Solid'], ['gradient', 'Gradient'], ['image', 'Image']] as [type, label]}
			<Button
				...
				onclick={() => (bgType = type as Settings['bgType'])}
			>
				{label}
			</Button>
		{/each}
		...
		{:else}
			<label class="block w-full">
				<input
					...
					class="hidden"
				/>
				<Button variant="secondary" size="sm" class="w-full pointer-events-none"
					>Upload Image</Button
				>
			</label>
		{/if}
