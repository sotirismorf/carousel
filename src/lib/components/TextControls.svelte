...
<script lang="ts">
	...
	let fontAvailable = $state<boolean | null>(null);
	let checkTimeout: ReturnType<typeof setTimeout> | null = null; // NOT reactive to avoid infinite loop

	function checkFontAvailability(fontName: string): void {
		...
		<div class="flex">
			{#each ['left', 'center', 'right', 'justify'] as a}
				<Button
					...
					onclick={() => (textAlign = a as Settings['textAlign'])}
				>
					{a[0].toUpperCase()}
				</Button>
			{/each}
		...
		<div class="flex">
			{#each [['top', 'T'], ['center', 'C'], ['bottom', 'B']] as [v, l]}
				<Button
					...
					size="sm"
					class="flex-1 rounded-none first:rounded-l-md last:rounded-r-md -ml-px first:ml-0 px-2"
					onclick={() => (verticalAlign = v as Settings['verticalAlign'])}
				>
					{l}
				</Button>
			{/each}
