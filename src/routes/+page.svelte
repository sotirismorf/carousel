<script lang="ts">
	...
	const scaledCorners = $derived(
		Object.fromEntries(
			Object.entries(settings.corners).map(([key, val]) => [
				key,
				{ ...val, size: val.size * settings.exportScale },
			])
		) as Record<string, CornerConfig>
	);
				...
				<div class="flex items-start w-max">
					{#each slides as html, i}
						...
						>
							{#if i > 0}
								<div
									class="absolute left-0 top-0 bottom-0 w-0 border-l-2 border-dashed border-white/30 -translate-x-[1px]"
								></div>
							{/if}
							<Slide
								{html}
